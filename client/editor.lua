...
-- ─────────────────────────────────────────────────────────────────────────────
--  MODEL LOADER  (replicado do modelkit)
-- ─────────────────────────────────────────────────────────────────────────────
...
local function LaserPickSingle(label, color)
    ...
    local hint = label .. '\n~g~[E]~w~ CONFIRMAR  ~r~[BACKSPACE]~w~ CANCELAR'
    ...
    while true do
        ...
        elseif IsControlJustReleased(0, 200) then   -- BACKSPACE
            lib.hideTextUI()
            return nil
        end
        Wait(0)
...
local function PedPickPosition()
    lib.showTextUI(
        'Vai para a posição pretendida\n~g~[ENTER]~w~ CONFIRMAR  ~r~[BACKSPACE]~w~ CANCELAR',
    ...
    while true do
        ...
        elseif IsControlJustPressed(0, 200) then -- BACKSPACE
            lib.hideTextUI()
            return nil, nil
        end
        Wait(0)
...
-- ─────────────────────────────────────────────────────────────────────────────
--  NPC PREVIEW PLACER  (replicado do modelkit/StartCreator)
--  Spawna ped com alpha 150, move com raycast, SHIFT/ALT roda, ENTER confirma
-- ─────────────────────────────────────────────────────────────────────────────
...
local function NpcPickPosition(pedModel)
    ...
    while true do
        ...
        if IsControlPressed(0, 19) then         -- SHIFT → rodar esquerda
            SetEntityHeading(ent, GetEntityHeading(ent) + 1.5)
        elseif IsControlPressed(0, 21) then     -- ALT   → rodar direita
            SetEntityHeading(ent, GetEntityHeading(ent) - 1.5)
        end
...
OpenEditorStartMenu = function()
    -- Contar restaurantes existentes
    local count = 0
                        ...
                        },
                        {
                            ...
                            onSelect    = function()
                                local confirm = lib.alertDialog({
                                    ...
                                    content  = string.format('Tens a certeza que queres eliminar **%s**?\n\nEsta ação é irreversível.', k),
                                    centered = true,
                                    cancel   = true,
                                ...
                                if confirm == 'confirm' then
                                    ...
                                    -- Remoção local imediata (servidor sincronizará via syncRestaurant)
                                    Restaurants[k] = nil
                                    OpenEditorStartMenu()
                                else
                                    OpenRestaurantListMenu()
...
OpenRouteMenu = function(routeKey, routeLabel)
    local route = editorData[routeKey]   -- referência direta
    ...
    if #route > 0 then
        ...
        options[#options + 1] = {
            ...
            onSelect    = function()
                ...
                if confirm == 'confirm' then
                    -- Limpar in-place para manter a referência
...
RegisterNetEvent('vc_restaurants:client:startEditor', function()
    if editorActive then
        -- Reabre o menu atual se fechou com ESC
