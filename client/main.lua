...
local spawned    = {}   -- [restaurantKey] = true (ped já solicitado)
local orderReady = {}   -- [restaurantKey] = bool (tabuleiro recolhido)

-- ─────────────────────────────────────────────────────────────────────────────
--  ZONE-BASED PROXIMITY  (substitui o loop de polling GetInteriorFromEntity)
--  Cada restaurante tem uma sphere zone — ao entrar na zona, solicita spawn do ped.
-- ─────────────────────────────────────────────────────────────────────────────

CreateThread(function()
    -- Pequeno wait para garantir que ox_lib está pronto
    Wait(500)
...
-- ─────────────────────────────────────────────────────────────────────────────
--  SYNC RESTAURANTS TABLE  (atualizado pelo server após save/delete no editor)
-- ─────────────────────────────────────────────────────────────────────────────
...
RegisterNetEvent('vc_restaurants:client:loadAnimation', function(animDict, animName, pedNetId, restaurantKey)
    ...
    CreateThread(function()
        ...
        -- Ped deixou de existir — limpar target
        exports.ox_target:removeEntity(pedNetId)
    end)
end)
...
RegisterNetEvent('vc_restaurants:client:prepareOrder', function(restaurantKey, itemName, pedNetId, buyerSource)
    ...
    -- Atualizar TextUI para "vai buscar o pedido"
    lib.hideTextUI()
    ...
    -- Target no tabuleiro para recolha
    ...
    -- Sinalizar ao servidor que este pedido terminou → processa o próximo na fila
